...
@router.get("/current")
async def get_current_tier(user_id: int):
    """
...
async def check_feature_access(feature: str, user_id: int):
    """
...
async def check_usage_limit(limit_name: str, user_id: int, current_usage: int = 0):
    """
...
async def get_upgrade_options(user_id: int):
    """
...
async def get_checkout_url(tier: str, user_id: int):
    """
    ...
    # Add user email/id to checkout for tracking
    db = SessionLocal()
    try:
        user = db.query(User).filter(User.id == user_id).first()
        if user:
            checkout_url = f"{checkout_url}?checkout[email]={user.email}&checkout[custom][user_id]={user_id}"
    finally:
        db.close()
...
@router.get("/usage/{user_id}/check/{action}")
async def check_usage_limit_action(user_id: int, action: str):
    """
...
@router.post("/usage/{user_id}/record/{action}")
...
@router.get("/usage/{user_id}/history/{action}")
