...
# T7: tokens are encrypted at rest with the shared Fernet infra. decrypt_field
# is legacy-tolerant — a pre-encryption plaintext row decrypts to itself and is
# re-encrypted on the next save_token(), so no big-bang migration is needed.
...
def load_token(api_type: str) -> dict:
    ...
    try:
        ...
        # Return token data (T7: decrypt on read; legacy plaintext passes through)
        return {
            "access_token": decrypt_field(token.access_token),
            "refresh_token": decrypt_field(token.refresh_token),
...
def get_token_status() -> dict:
    ...
    def format_status(token_data):
        ...
        now = datetime.utcnow()  # naive, matching the naive expires_at/obtained_at
        ...
        return {
            ...
            # T7: the access_token_preview (first 20 chars of the raw token)
            # was removed — status must never echo any part of the secret.
        }
