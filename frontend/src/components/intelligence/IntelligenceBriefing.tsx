...
export const IntelligenceBriefing: React.FC = () => {
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      ...
      case 'medium': return 'bg-yellow-100 text-amber-300 border-yellow-300';
      case 'low': return 'bg-blue-100 text-blue-800 border-blue-300';
      default: return 'bg-gray-100 text-primary border-white/10';
    }
  };

  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case 'high': return 'bg-red-500/100 text-white';
      case 'medium': return 'bg-amber-500/100 text-white';
      case 'low': return 'bg-green-500/100 text-white';
      default: return ' 0 text-white';
    }
  };
  ...
  return (
    <GlassPanel depth={60} delay={0.3}>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-light text-primary">Intelligence Briefing</h2>
        ...
        <div className="space-y-6">
          ...
          {briefing.key_metrics && (
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="bg-green-500/100/10 rounded-lg p-4">
          ...
          <div>
            ...
            <div className="  rounded-lg p-4 prose max-w-none">
              <p className="text-secondary whitespace-pre-wrap">{briefing.briefing_text}</p>
            </div>
          </div>
          ...
          {briefing.recommended_actions && briefing.recommended_actions.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-primary mb-3">Recommended Actions</h3>
              <div className="space-y-2">
                {briefing.recommended_actions.map((action, index) => (
                  <div
                    key={index}
                    className="border border-white/10 rounded-lg p-4 hover:border-blue-300 transition"
                  >
