...
export const OrdersPage: React.FC = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'shipped': return 'bg-cyan-500/100/20 text-blue-600';
      case 'unfulfilled': return 'bg-amber-500/100/20 text-yellow-600';
      case 'delivered': return 'bg-green-500/100/20 text-green-600';
      default: return ' 0/20 text-secondary';
    }
  };

  if (loading) {
    return (
      <div className="min-h-screen   p-12 flex items-center justify-center" style={{ perspective: '1500px' }}>
        <GlassPanel depth={60} delay={0}>
          <div className="text-center py-12">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-gray-900 mx-auto mb-4"></div>
            <p className="text-secondary font-light">Loading orders...</p>
          </div>
        </GlassPanel>
      </div>
    );
    ...
    <div className="min-h-screen   p-12" style={{ perspective: '1500px' }}>
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <GlassPanel depth={80} delay={0.1}>
          <div>
            <h2 className="text-3xl font-light text-primary mb-2">
              Order Management
            </h2>
            <p className="text-secondary">Track and fulfill customer orders</p>
          </div>
        </GlassPanel>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <GlassPanel depth={70} delay={0.15}>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-2xl font-light text-primary">{stats.total}</span>
            </div>
          ...
          <GlassPanel depth={70} delay={0.2}>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-2xl font-light text-primary">{stats.pending}</span>
            </div>
          ...
          <GlassPanel depth={70} delay={0.25}>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-2xl font-light text-primary">{stats.shipped}</span>
            </div>
            <p className="text-secondary text-sm">Shipped</p>
          ...
          <GlassPanel depth={70} delay={0.3}>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-2xl font-light text-primary">${stats.revenue.toFixed(2)}</span>
            </div>
        ...
        {orders.length === 0 ? (
          <GlassPanel depth={60} delay={0.4}>
            <div className="text-center py-20">
              <ShoppingCart className="w-16 h-16 text-tertiary mx-auto mb-4 opacity-30" />
              <h3 className="text-xl font-light text-secondary">No Orders Yet</h3>
              <p className="text-tertiary mt-1">New orders from your stores will appear here.</p>
            </div>
          </GlassPanel>
        ) : (
              ...
              <GlassPanel key={order.id} depth={55} delay={0.35 + idx * 0.02}>
                <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-center">
                  <div className="md:col-span-2">
                    <p className="font-light text-lg text-primary">#{order.shopify_order_number}</p>
                    <p className="text-sm text-secondary flex items-center gap-2"><User size={14}/> {order.customer_name}</p>
                  </div>
                  <div className="md:col-span-2">
                    <p className="font-light text-primary flex items-center gap-2"><Package size={14} /> {order.product_name}</p>
                    <p className="text-sm text-tertiary">Qty: {order.quantity}</p>
                  </div>
                  ...
                  <div className="flex flex-col items-start md:items-end gap-2">
                    ...
                    <p className="text-xs text-tertiary flex items-center gap-1"><Calendar size={12} /> {new Date(order.created_at).toLocaleDateString()}</p>
                  </div>
          ...
          <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setSelectedOrder(null)}>
            <div className="glass p-8 max-w-md w-full" style={{ boxShadow: '0 40px 120px rgba(0, 0, 0, 0.25)' }} onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-light text-primary">Add Tracking</h2>
                <button onClick={() => setSelectedOrder(null)} className="p-1.5 rounded-full hover:bg-gray-900/10 text-tertiary hover:text-primary transition">
                  <X size={20}/>
                </button>
              </div>

              <div className="mb-6 bg-gray-100/50 p-4 rounded-lg">
                <p className="text-sm text-secondary">Order #{selectedOrder.shopify_order_number}</p>
                <p className="font-light text-primary">{selectedOrder.product_name}</p>
              </div>

              <div className="space-y-4">
                {/* Inputs with labels */}
                <div>
                  <label className="block text-sm font-light text-secondary mb-2">Tracking Number</label>
                  <input
                    type="text"
                    value={trackingNumber}
                    onChange={(e) => setTrackingNumber(e.target.value)}
                    placeholder="e.g., 1Z999AA10123456784"
                    className="w-full bg-white/50 border border-white/10 rounded-lg px-4 py-2.5 text-primary placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-900"
                  />
                </div>
                <div>
                  <label className="block text-sm font-light text-secondary mb-2">Tracking URL</label>
                  <input
                    type="url"
                    value={trackingUrl}
                ...
                <div>
                  <label className="block text-sm font-light text-secondary mb-2">Carrier</label>
                  <select
                    value={trackingCompany}
                    onChange={(e) => setTrackingCompany(e.target.value)}
                    className="w-full bg-white/50 border border-white/10 rounded-lg px-4 py-2.5 text-primary appearance-none focus:outline-none focus:ring-2 focus:ring-gray-900"
                  >
                    <option>Other</option>
                    <option>USPS</option>
                    <option>UPS</option>
              ...
              <div className="flex gap-3 mt-8">
                <button
                  ...
                  className="flex-1 text-secondary py-2.5 px-4 rounded-lg hover:bg-gray-900/5 font-light transition"
                >
                  Cancel
                </button>
                <button
