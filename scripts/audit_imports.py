#!/usr/bin/env python3
"""
Import Audit Script for Ospra OS
Scans the entire codebase for broken imports and missing files.
"""
...
class ImportAuditor:
    ...
    def scan_project(self) -> List[ImportIssue]:
        ...
        print(f"Found {len(self.all_modules)} Python modules")
    ...
    def _collect_modules(self):
        """Collect all Python module paths in the project."""
    ...
    def _is_external_module(self, module_name: str) -> bool:
        ...
        external_prefixes = [
            ...
            "framer", "lucide", "react", "zustand", "tanstack",
    ...
    def generate_report(self) -> str:
        ...
        for issue_type, issues in sorted(by_type.items()):
            ...
            for issue in issues:
                report.append(f"- **{issue.file_path}** (line {issue.line_number})")
    ...
    def generate_fix_script(self) -> str:
        ...
        script = ["#!/usr/bin/env python3", '"""Auto-generated import fix script."""', ""]

        # Group missing modules
        missing = [i for i in self.issues if i.issue_type == "missing_module"]

        if missing:
            script.append("# Files with missing imports that need manual review:")
            for issue in missing:
                script.append(f"# - {issue.file_path}: {issue.import_statement}")
...
def main():
    ...
    # Check for ospra_os directory
    if (project_root / "ospra_os").exists():
        scan_dir = project_root
    elif (project_root.parent / "ospra_os").exists():
        scan_dir = project_root.parent
    else:
        print("Could not find ospra_os directory")
        sys.exit(1)

    auditor = ImportAuditor(scan_dir)
    ...
    # Save report
    report_path = scan_dir / "import_audit_report.md"
