...
def test_api_endpoints():
    ...
    try:
        ...
        if data.get('success'):
            ...
            print(f"[WARNING]  No velocity data yet")
    except Exception as e:
        print(f"[ERROR] Velocity test failed: {e}")

    # Test 3: Smart Recommendations
    print("\n3⃣ Testing Smart Recommendations...")
    try:
        r = requests.post(
        ...
        data = r.json()
        if data.get('success'):
            print(f"[SUCCESS] Smart Recommendations:")
            ...
            print(f"[WARNING]  Recommendations failed: {data.get('error')}")
    except Exception as e:
        print(f"[ERROR] Recommendations test failed: {e}")

    # Test 4: Analytics
    print("\n4⃣ Testing Recommendation Analytics...")
    try:
        r = requests.get(f"{base_url}/api/recommendations/analytics/1")
        data = r.json()
        if data.get('success'):
            print(f"[SUCCESS] Analytics:")
            ...
            print(f"[WARNING]  No analytics data yet")
    except Exception as e:
        print(f"[ERROR] Analytics test failed: {e}")

    # Test 5: Auto-Discovery Manual Trigger
    print("\n5⃣ Testing Auto-Discovery (Manual)...")
    try:
        r = requests.post(f"{base_url}/api/admin/run-discovery-now")
        data = r.json()
        if data.get('success'):
            print(f"[SUCCESS] Auto-discovery completed")
        else:
            print(f"[ERROR] Discovery failed: {data.get('error')}")
    except Exception as e:
