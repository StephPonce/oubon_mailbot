"""
🧪 Test Priority-Based Product Discovery System
...
def test_priority_system():
    print("\n" + "="*60)
    print("🧪 TESTING PRIORITY-BASED PRODUCT DISCOVERY")
    print("="*60 + "\n")
    ...
    print("1️⃣ Checking AliExpress API initialization...")
    if discovery.aliexpress_api:
        print("   ✅ AliExpress API client is initialized")
        print(f"   📌 App Key: {discovery.aliexpress_api.app_key}")
        print(f"   📌 Gateway: {discovery.aliexpress_api.gateway}")
    else:
        print("   ⚠️  AliExpress API client not available")
        print("   💡 Will fall back to scraping or template data")

    print("\n2️⃣ Discovering products for 'smart_home' niche...")
    ...
    print("="*60)
    print(f"✅ Data Source Used: {result['data_source']}")
