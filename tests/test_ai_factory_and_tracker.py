...
def test_ai_factory():
    ...
    try:
        ...
        print(f"❌ Provider retrieval failed: {e}")
        return
    ...
    try:
        ...
        print(f"✅ Created Claude provider: {claude.provider_name}")

        gemini = AIFactory.get_provider("gemini", api_key="test-key")
        print(f"✅ Created Gemini provider: {gemini.provider_name}")
        ...
        try:
            ...
            print(f"✅ Correctly rejected invalid provider")
    except Exception as e:
    ...
    try:
        ...
        print(f"✅ Cost comparison for 100,000 tokens:")
        for p in comparison:
            print(f"   {p['display_name']}: ${p['monthly_cost']:.2f}/month")
    except Exception as e:
        print(f"❌ compare_providers() failed: {e}")
        return
    ...
    try:
        ...
        print(f"✅ Cost priority recommendation: {rec_cost['provider']}")
        ...
        print(f"✅ Quality priority recommendation: {rec_quality['provider']}")
        ...
        print(f"✅ Balanced recommendation: {rec_balanced['provider']}")
        ...
        print(f"✅ Budget-constrained ($1) recommendation: {rec_budget['provider']}")
        if 'warning' in rec_budget:
            print(f"   ⚠️  {rec_budget['warning']}")
    except Exception as e:
        print(f"❌ recommend_provider() failed: {e}")
        return
    ...
    try:
        ...
        print(f"✅ Created provider via convenience function: {ai.provider_name}")
    except Exception as e:
        print(f"❌ get_ai_provider() failed: {e}")
        return

    print("\n" + "=" * 70)
    print("✅ AI FACTORY TESTS PASSED!")
    print("=" * 70)
    print()
...
def test_cost_tracker():
    ...
    try:
        ...
        print("✅ Created tracker via constructor")

        tracker2 = get_cost_tracker()
        print("✅ Created tracker via convenience function")
    except Exception as e:
    ...
    try:
        ...
        print("✅ track_usage() method signature correct")
        ...
        print(f"❌ track_usage() interface check failed: {e}")
    ...
    try:
        ...
        print("✅ get_user_costs(user_id, days) signature correct")

        # Check get_usage_stats signature
        sig = inspect.signature(tracker1.get_usage_stats)
        params = list(sig.parameters.keys())
        assert 'user_id' in params
        print("✅ get_usage_stats(user_id) signature correct")
        ...
        print("✅ check_budget_alert(user_id, monthly_budget) signature correct")

        # Check get_provider_comparison signature
        sig = inspect.signature(tracker1.get_provider_comparison)
        params = list(sig.parameters.keys())
        assert 'user_id' in params
        print("✅ get_provider_comparison(user_id) signature correct")
    ...
    print("\n" + "=" * 70)
    print("✅ COST TRACKER TESTS PASSED!")
    print("=" * 70)
    print()
...
def test_integration():
    ...
    try:
        ...
        for i, p in enumerate(comparison, 1):
            ...
            if i < len(comparison):
                print(f"   💰 Save ${savings:.2f}/month vs most expensive option")
        print()

        # Show what features cost tracker provides
        print("Cost Tracker Features:")
        print("✓ Track every AI API call automatically")
        print("✓ Monitor costs per user, provider, and task type")
        print("✓ Budget alerts when approaching limits")
        print("✓ Provider comparison to find savings")
